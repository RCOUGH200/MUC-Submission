...
public class mcMapDataDBMgr extends SQLiteOpenHelper {
    ...
    private static final String DB_NAME = "tournamentLoc.s3db";
    private static final String TBL_TLOCATION = "mapTournLoc";

    public static final String COL_ENTRYID = "entryID";
    public static final String COL_TOURNAMENT = "Name";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String CREATE_MAPEKFAME_TABLE = "CREATE TABLE IF NOT EXISTS " +
                TBL_TLOCATION + "("
                + COL_ENTRYID + " INTEGER PRIMARY KEY," + COL_TOURNAMENT
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        {
            db.execSQL("DROP TABLE IF EXISTS " + TBL_TLOCATION);
            onCreate(db);
        }
    }
    ...
    public void addaMapEKFameEntry(mcMapData aMapEKFame) {
        ...
        values.put(COL_TOURNAMENT, aMapEKFame.getTournament());
        values.put(COL_LATITUDE, aMapEKFame.getLatitude());
        values.put(COL_LONGITUDE, aMapEKFame.getLongitude());

        SQLiteDatabase db = this.getWritableDatabase();

        db.insert(TBL_TLOCATION, null, values);
        db.close();
    }

    public mcMapData getMapEKFameEntry(String aMapEKFameEntry) {
        String query = "Select * FROM " + TBL_TLOCATION + " WHERE " + COL_TOURNAMENT + " =  \"" + aMapEKFameEntry + "\"";
        ...
        if (cursor.moveToFirst()) {
            ...
            MapDataEntry.setTournament(cursor.getString(1));
...
/*
        if (cursor.moveToFirst()) {
            ...
            db.delete(TBL_TLOCATION, COL_ENTRYID + " = ?",
